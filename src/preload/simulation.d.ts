...
// ────────────────────────────────────────────────────────────────────────
// Analysis plan (worker Step 1, in-memory only)
// ────────────────────────────────────────────────────────────────────────

/**
 * One pair of outcomes the model was asked to compare in the per-outcome
 * analyses. The `outcomes` list references outcome ids; the `focus`
 * string is a short instruction the model should respect when
 * emitting the per-outcome comparison deltas.
 */
export interface AnalysisPlanComparison {
  outcomes: string[]
  focus: string
}

/**
 * The planning-pass output. Generated once per simulation by the
 * worker (Step 1 of the 2-step pipeline) and held in memory in
 * `src/main/planCache.ts`; never persisted to `simulation.json`.
 *
 * - `summary` — one-sentence user-facing description of what the
 *   analysis aims to compare. Generated by the model.
 * - `comparisons` — pairs of outcome ids the per-outcome step should
 *   produce comparison deltas for.
 * - `dimensions` — the list of fields every per-outcome JSON must
 *   populate (e.g. summary, risk, severeCaseRate, keyDrivers, ...).
 * - `hypotheses` — 2–3 claims the model should test as it analyses
 *   each outcome.
 */
export interface AnalysisPlan {
  summary: string
  comparisons: AnalysisPlanComparison[]
  dimensions: string[]
  hypotheses: string[]
}
